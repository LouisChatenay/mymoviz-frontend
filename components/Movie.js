        ...
        <div>
          ...
          {isTruncated && 
            <TextTruncate
              ...
              truncateText="…"
              text= {props.overview}
              textTruncateChild={<a onClick={() => setIsTruncated(false)}>Read on</a>}
          />}
          {!isTruncated &&
            <>
              <p className={styles.untruncatedDescription}>{props.overview}</p> 
              <a onClick={() => setIsTruncated(true)}><span>Hide</span></a>
            </>
          }
        </div>
